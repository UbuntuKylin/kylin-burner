...
#define BURNER_METADATA_SILENCE_INTERVAL		100000000LL
#define BURNER_METADATA_INITIAL_STATE			GST_STATE_PAUSED

struct BurnerMetadataPrivate {
	GstElement *pipeline;
	GstElement *source;
	GstElement *decode;
	GstElement *convert;
	...
	BurnerMetadataGetXidCb xid_callback;
...
typedef struct BurnerMetadataPrivate BurnerMetadataPrivate;
#define BURNER_METADATA_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), BURNER_TYPE_METADATA, BurnerMetadataPrivate))
...
typedef enum {
	COMPLETED_SIGNAL,
	LAST_SIGNAL
} BurnerMetadataSignalType;

static guint burner_metadata_signals [LAST_SIGNAL] = { 0 };

#define BURNER_METADATA_IS_FAST(flags) 					\
	(!((flags) & BURNER_METADATA_FLAG_SILENCES) &&				\
	((flags) & BURNER_METADATA_FLAG_FAST))

G_DEFINE_TYPE (BurnerMetadata, burner_metadata, G_TYPE_OBJECT)

static GSList *downloading = NULL;
static GSList *downloaded = NULL;

static gboolean
burner_metadata_completed (BurnerMetadata *self);

static int
burner_metadata_get_xid (BurnerMetadata *metadata)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (metadata);
...
struct _BurnerMetadataGstDownload {
	gchar *detail;

	/* These are all metadata objects waiting */
	GSList *objects;
};
typedef struct _BurnerMetadataGstDownload BurnerMetadataGstDownload;
...
burner_metadata_info_free (BurnerMetadataInfo *info)
{
	...
	burner_metadata_info_clear (info);
			    ...
			    BurnerMetadataInfo *src)
{
	...
	for (iter = src->silences; iter; iter = iter->next) {
		BurnerMetadataSilence *silence, *copy;

		silence = iter->data;

		copy = g_new0 (BurnerMetadataSilence, 1);
...
static void
burner_metadata_stop_pipeline (GstElement *pipeline)
{
	GstState state;
	...
	while (change == GST_STATE_CHANGE_ASYNC && state != GST_STATE_NULL) {
		...
		change = gst_element_get_state (pipeline,
						...
						&pending,
						GST_MSECOND);
		BURNER_UTILS_LOG ("Get state (current = %i pending = %i) returned %i",
...
static void
burner_metadata_destroy_pipeline (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (self);

	priv->started = 0;

	if (priv->pipeline_mp3) {
		burner_metadata_stop_pipeline (priv->pipeline_mp3);
	...
	burner_metadata_stop_pipeline (priv->pipeline);
...
static void
burner_metadata_stop (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	GSList *iter;
	...
	BURNER_UTILS_LOG ("Retrieval ended for %s %p",
	...
	if (priv->pipeline)
		burner_metadata_destroy_pipeline (self);
	...
	if (priv->downloads) {
		...
		for (iter = priv->downloads; iter; iter = iter->next) {
			BurnerMetadataGstDownload *download;
...
void
burner_metadata_cancel (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (self);

	BURNER_UTILS_LOG ("Metadata retrieval cancelled for %s %p",
			   priv->info ? priv->info->uri:"Unknown",
			   self);
	
	burner_metadata_stop (self);
	if (priv->error) {
...
static void
burner_metadata_install_plugins_add_downloaded (GSList *downloads)
{
	GSList *iter;
...
static void
burner_metadata_install_plugins_free_data (GSList *downloads)
{
	GSList *iter;
	...
	for (iter = downloads; iter; iter = iter->next) {
		...
		for (meta = download->objects; meta; meta = meta->next) {
			...
			priv = BURNER_METADATA_PRIVATE (meta->data);
...
static void
burner_metadata_install_plugins_success (BurnerMetadataGstDownload *download)
{
	GSList *iter;
	...
	for (iter = download->objects; iter; iter = iter->next) {
		...
		priv = BURNER_METADATA_PRIVATE (iter->data);
...
static void
burner_metadata_install_plugins_abort (BurnerMetadataGstDownload *download)
{
	GSList *iter;
	GSList *next;
	...
	for (iter = download->objects; iter; iter = next) {
		...
		burner_metadata_completed (BURNER_METADATA (iter->data));
	}
}

static void
burner_metadata_install_plugins_completed (BurnerMetadataGstDownload *download)
{
	GSList *iter;
	GSList *next;
...
static void
burner_metadata_install_plugins_result (GstInstallPluginsReturn res,
					 gpointer data)
{
	...
	case GST_INSTALL_PLUGINS_SUCCESS:
		burner_metadata_install_plugins_add_downloaded (downloads);
		...
		for (iter = downloads; iter; iter = iter->next) {
			...
			download = iter->data;
			burner_metadata_install_plugins_success (download);
		}
		break;
	...
	case GST_INSTALL_PLUGINS_NOT_FOUND:
		...
		/* stop everything */
		for (iter = downloads; iter; iter = iter->next)
			burner_metadata_install_plugins_completed (iter->data);
		break;
	...
	case GST_INSTALL_PLUGINS_USER_ABORT:
		...
		for (iter = downloads; iter; iter = iter->next) {
			...
			download = iter->data;
			burner_metadata_install_plugins_abort (download);
		}
		break;
	...
	burner_metadata_install_plugins_free_data (downloads);
}

static BurnerMetadataGstDownload *
burner_metadata_is_downloading (const gchar *detail)
{
	GSList *iter;
...
static gboolean
burner_metadata_install_missing_plugins (BurnerMetadata *self)
{
	...
	BURNER_UTILS_LOG ("Starting to download missing plugins");
	...
	for (iter = priv->missing_plugins; iter; iter = iter->next) {
		...
		download = burner_metadata_is_downloading (detail);
		if (download) {
		...
		download = g_new0 (BurnerMetadataGstDownload, 1);
	...
	gst_install_plugins_context_set_xid (context, burner_metadata_get_xid (self));
	status = gst_install_plugins_async ((gchar **) details->pdata,
					    context,
					    burner_metadata_install_plugins_result,
					    downloads);

	gst_install_plugins_context_free (context);
	g_ptr_array_free (details, TRUE);

	BURNER_UTILS_LOG ("Download status %i", status);
...
burner_metadata_completed (BurnerMetadata *self)
{
	...
	if (priv->error) {
		BURNER_UTILS_LOG ("Operation completed with an error %s", priv->error->message);
	}

	/* See if we have missing plugins */
	if (priv->missing_plugins) {
		if (burner_metadata_install_missing_plugins (self))
			return TRUE;
	}
	...
	g_signal_emit (G_OBJECT (self),
		       burner_metadata_signals [COMPLETED_SIGNAL],
		       0,
		       priv->error);
...
static gboolean
burner_metadata_thumbnail (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	gint64 position;
	gboolean res;
	...
	BURNER_UTILS_LOG ("Seeking forward %i for %s", res, priv->info->uri);
	if (!res)
		return burner_metadata_completed (self);
...
static void
burner_metadata_is_seekable (BurnerMetadata *self)
{
	GstQuery *query;
	GstFormat format;
	gboolean seekable;
...
static gboolean
burner_metadata_get_mime_type (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	GstElement *typefind;
	GstCaps *caps = NULL;
	...
	BURNER_UTILS_LOG ("Mime type %s", mime);
	...
	if (!strcmp (mime, "application/x-id3"))
		...
		BURNER_UTILS_LOG ("Wav file has dts: %s", priv->info->has_dts? "yes":"no");
	}
	else
		priv->info->type = g_strdup (mime);
...
static gboolean
burner_metadata_is_mp3 (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	...
	if (!priv->info->type
	&&  !burner_metadata_get_mime_type (self))
		return FALSE;
...
static gboolean
burner_metadata_process_element_messages (BurnerMetadata *self,
					   GstMessage *msg)
{
	BurnerMetadataPrivate *priv;
	...
	||  gst_message_has_name (msg, "pixbuf")) {
		...
		BURNER_UTILS_LOG ("Received pixbuf snapshot sink (%p) for %s", priv->info->snapshot, priv->info->uri);
	...
	if ((priv->flags & BURNER_METADATA_FLAG_MISSING)
	...
	&&   gst_structure_has_field (s, "peak")) {
		...
		if (peak < -50.0) {
			...
			if (pos == -1) {
				BURNER_UTILS_LOG ("impossible to retrieve position");
				return TRUE;
			}

			if (!priv->silence) {
				priv->silence = g_new0 (BurnerMetadataSilence, 1);
			...
			BURNER_UTILS_LOG ("silence detected at %lli", pos);
		}
		else if (priv->silence) {
			BURNER_UTILS_LOG ("silence finished");
...
static void
burner_metadata_process_pending_messages (BurnerMetadata *self)
{
	GstBus *bus;
	GstMessage *msg;
	BurnerMetadataPrivate *priv;
	...
	while ((msg = gst_bus_pop (bus))) {
		...
		if (GST_MESSAGE_TYPE (msg) == GST_MESSAGE_TAG) {
			...
			burner_metadata_process_element_messages (self, msg);
...
static gboolean
burner_metadata_success (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (self);

	BURNER_UTILS_LOG ("Metadata retrieval completed for %s", priv->info->uri);

	/* check if that's a seekable one */
	burner_metadata_is_seekable (self);
	...
	&& !priv->snapshot_started)
		return burner_metadata_thumbnail (self);

	return burner_metadata_completed (self);
}

static gboolean
burner_metadata_get_duration (BurnerMetadata *self,
			       GstElement *pipeline,
			       gboolean use_duration)
{
	...
	if (duration == -1) {
		if (!priv->error) {
			...
			BURNER_GET_BASENAME_FOR_DISPLAY (priv->info->uri, name);
			priv->error = g_error_new (BURNER_UTILS_ERROR,
	...
	BURNER_UTILS_LOG ("Found duration %lli for %s", duration, priv->info->uri);

	priv->info->len = duration;
	return burner_metadata_success (self);
}
...
static gboolean
burner_metadata_mp3_bus_messages (GstBus *bus,
				   GstMessage *msg,
				   BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	...
	case GST_MESSAGE_ERROR:
		...
		BURNER_UTILS_LOG ("GStreamer error - mp3 - (%s)", debug_string);
		g_free (debug_string);
		if (!priv->error && error)
			priv->error = error;

		burner_metadata_completed (self);
		return FALSE;

	case GST_MESSAGE_EOS:
		BURNER_UTILS_LOG ("End of stream reached - mp3 - for %s", priv->info->uri);
		burner_metadata_get_duration (self, priv->pipeline_mp3, FALSE);
		return FALSE;
...
static gboolean
burner_metadata_create_mp3_pipeline (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	GstElement *source;
	GstElement *parse;
	...
	priv->watch_mp3 = gst_bus_add_watch (bus,
					     (GstBusFunc) burner_metadata_mp3_bus_messages,
					     self);
...
static gboolean
burner_metadata_success_main (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (self);

	BURNER_UTILS_LOG ("Metadata retrieval successfully completed for %s", priv->info->uri);

	/* find the type of the file */
	burner_metadata_get_mime_type (self);

	/* empty the bus of any pending message */
	burner_metadata_process_pending_messages (self);

	/* get the size */
	if (burner_metadata_is_mp3 (self)) {
		if (!burner_metadata_create_mp3_pipeline (self)) {
			BURNER_UTILS_LOG ("Impossible to run mp3 specific pipeline");
			return burner_metadata_completed (self);
		}
	...
	return burner_metadata_get_duration (self, priv->pipeline, TRUE);
}

static gboolean
burner_metadata_bus_messages (GstBus *bus,
			       GstMessage *msg,
			       BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	...
	case GST_MESSAGE_ELEMENT:
		return burner_metadata_process_element_messages (self, msg);

	case GST_MESSAGE_ERROR:
		/* save the error message */
		gst_message_parse_error (msg, &error, &debug_string);
		BURNER_UTILS_LOG ("GStreamer error (%s)", debug_string);
		g_free (debug_string);
		if (!priv->error && error)
			priv->error = error;
	...
	case GST_MESSAGE_EOS:
		BURNER_UTILS_LOG ("End of stream reached for %s", priv->info->uri);
		return burner_metadata_success_main (self);
...
static gboolean
burner_metadata_create_audio_pipeline (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	GstPad *audio_pad;
	...
	if (priv->flags & BURNER_METADATA_FLAG_SILENCES) {
		...
		if (!priv->level) {
			...
			g_object_set (priv->level,
				      "message", TRUE,
				      "interval", (guint64) BURNER_METADATA_SILENCE_INTERVAL,
				      NULL);
		}
		...
		if (!gst_element_link_many (priv->convert,
		                            ...
		                            priv->sink,
		                            NULL)) {
			BURNER_UTILS_LOG ("Impossible to link elements");
			gst_object_unref (priv->audio);
			priv->audio = NULL;
			return FALSE;
		}
	...
	else if (priv->flags & BURNER_METADATA_FLAG_THUMBNAIL) {
		GstElement *queue;
	...
	BURNER_UTILS_LOG ("Adding audio pipeline for %s", priv->info->uri);

	return TRUE;
}

static gboolean
burner_metadata_create_video_pipeline (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	...
	if (!priv->snapshot) {
		...
		BURNER_UTILS_LOG ("gdkpixbufsink is not installed");
		return FALSE;
	}
	...
	if (!colorspace) {
		...
		BURNER_UTILS_LOG ("videoconvert is not installed");
		return FALSE;
	}
	...
	BURNER_UTILS_LOG ("Adding pixbuf snapshot sink for %s", priv->info->uri);

	return TRUE;
}

static void
burner_metadata_error_on_pad_linking (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	GstMessage *message;
	GstBus *bus;
	...
	message = gst_message_new_error (GST_OBJECT (priv->pipeline),
					 priv->error,
					 "Sent by burner_metadata_error_on_pad_linking");
...
static gboolean
burner_metadata_link_dummy_pad (BurnerMetadata *self,
				 GstPad *pad)
{
	BurnerMetadataPrivate *priv;
	GstElement *fakesink;
	...
	BURNER_UTILS_LOG ("Linking to a fake sink");
	...
	if (res == GST_PAD_LINK_OK) {
		gst_element_set_state (fakesink, BURNER_METADATA_INITIAL_STATE);
		return TRUE;
	}
...
static void
burner_metadata_audio_caps (BurnerMetadata *self,
                             GstCaps *caps)
{
	int i;
	int num_caps;
	...
	for (i = 0; i < num_caps; i++) {
		...
		if (gst_structure_has_field (structure, "channels")) {
			if (gst_structure_get_field_type (structure, "channels") == G_TYPE_INT) {
				...
				BURNER_UTILS_LOG ("Number of channels %i", priv->info->channels);
			}
				...
				BURNER_UTILS_LOG ("Unhandled type for channel prop %s",
		...
		if (gst_structure_has_field (structure, "rate")) {
			if (gst_structure_get_field_type (structure, "rate") == G_TYPE_INT) {
				...
				BURNER_UTILS_LOG ("Rate %i", priv->info->rate);
			}
				...
				BURNER_UTILS_LOG ("Unhandled type for rate prop %s",
...
static void
burner_metadata_new_decoded_pad_cb (GstElement *decode,
				     GstPad *pad,
				     BurnerMetadata *self)
{
	GstPad *sink;
	...
	BURNER_UTILS_LOG ("New pad for %s", priv->info->uri);
	...
	if (has_audio && !priv->audio_linked) {
		burner_metadata_audio_caps (self, caps);
		burner_metadata_create_audio_pipeline (self);
		sink = gst_element_get_static_pad (priv->audio, "sink");
		if (sink && !GST_PAD_IS_LINKED (sink)) {
			res = gst_pad_link (pad, sink);
			BURNER_UTILS_LOG ("Audio stream link %i for %s", res, priv->info->uri);
			gst_object_unref (sink);

			priv->audio_linked = (res == GST_PAD_LINK_OK);
			gst_element_set_state (priv->audio, BURNER_METADATA_INITIAL_STATE);
		}
	}

	if (!strcmp (name, "video/x-raw") && !priv->video_linked) {
		BURNER_UTILS_LOG ("RAW video stream found");

		if (!priv->video && (priv->flags & BURNER_METADATA_FLAG_THUMBNAIL)) {
			...
			if (!burner_metadata_create_video_pipeline (self)) {
				BURNER_UTILS_LOG ("Impossible to create video pipeline");

				gst_caps_unref (caps);

				if (!burner_metadata_link_dummy_pad (self, pad))
					burner_metadata_error_on_pad_linking (self);
			...
			gst_element_set_state (priv->video, BURNER_METADATA_INITIAL_STATE);

			BURNER_UTILS_LOG ("Video stream link %i for %s", res, priv->info->uri);
		}
		else if (!burner_metadata_link_dummy_pad (self, pad))
			burner_metadata_error_on_pad_linking (self);
	}
	else if (has_video && !burner_metadata_link_dummy_pad (self, pad))
		burner_metadata_error_on_pad_linking (self);
...
static gboolean
burner_metadata_create_pipeline (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
	...
	g_signal_connect (G_OBJECT (priv->decode), "pad-added",
			  G_CALLBACK (burner_metadata_new_decoded_pad_cb),
			  self);
...
static gboolean
burner_metadata_set_new_uri (BurnerMetadata *self,
			      const gchar *uri)
{
	BurnerMetadataPrivate *priv;
	GstBus *bus;
	...
	BURNER_UTILS_LOG ("New retrieval for %s %p", uri, self);
	...
	burner_metadata_info_free (priv->info);
	priv->info = NULL;
	...
	priv->info = g_new0 (BurnerMetadataInfo, 1);
	...
	else if (!burner_metadata_create_pipeline (self))
		return FALSE;
	...
	priv->watch = gst_bus_add_watch (bus,
					 (GstBusFunc) burner_metadata_bus_messages,
					 self);
	gst_object_unref (bus);
...
gboolean
burner_metadata_set_uri (BurnerMetadata *self,
			  BurnerMetadataFlag flags,
			  const gchar *uri,
			  GError **error)
{
	...
	priv->flags = flags;
	if (!burner_metadata_set_new_uri (self, uri)) {
		if (priv->error) {
			BURNER_UTILS_LOG ("Failed to set new URI %s", priv->error->message);
...
gboolean
burner_metadata_get_info_async (BurnerMetadata *self,
				 const gchar *uri,
				 BurnerMetadataFlag flags)
{
	BurnerMetadataPrivate *priv;
...
static void
burner_metadata_wait_cancelled (GCancellable *cancel,
				 GCond *condition)
{
	BURNER_UTILS_LOG ("Thread waiting for retrieval end cancelled");
	g_cond_broadcast (condition);
}

void
burner_metadata_wait (BurnerMetadata *self,
		       GCancellable *cancel)
{
	BurnerMetadataPrivate *priv;
	GCond *condition;
	...
	BURNER_UTILS_LOG ("Metadata lock and wait %p", self);
	...
	sig = g_signal_connect (cancel,
				"cancelled",
				G_CALLBACK (burner_metadata_wait_cancelled),
				condition);
...
void
burner_metadata_increase_listener_number (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
...
gboolean
burner_metadata_decrease_listener_number (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
...
const gchar *
burner_metadata_get_uri (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
...
BurnerMetadataFlag
burner_metadata_get_flags (BurnerMetadata *self)
{
	BurnerMetadataPrivate *priv;
...
gboolean
burner_metadata_get_result (BurnerMetadata *self,
			     BurnerMetadataInfo *info,
			     GError **error)
{
	BurnerMetadataPrivate *priv;
	...
	memset (info, 0, sizeof (BurnerMetadataInfo));
	burner_metadata_info_copy (info, priv->info);
	return TRUE;
}

static void
burner_metadata_init (BurnerMetadata *obj)
{
	BurnerMetadataPrivate *priv;

	priv = BURNER_METADATA_PRIVATE (obj);

	priv->mutex = g_mutex_new ();
}

static void
burner_metadata_finalize (GObject *object)
{
	BurnerMetadataPrivate *priv;
	GSList *iter;

	priv = BURNER_METADATA_PRIVATE (object);

	burner_metadata_destroy_pipeline (BURNER_METADATA (object));
	...
	G_OBJECT_CLASS (burner_metadata_parent_class)->finalize (object);
}

static void
burner_metadata_get_property (GObject *obj,
			       guint prop_id,
			       GValue *value,
			       GParamSpec *pspec)
{
...
static void
burner_metadata_set_property (GObject *obj,
			       guint prop_id,
			       const GValue *value,
			       GParamSpec *pspec)
{
...
static void
burner_metadata_class_init (BurnerMetadataClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	g_type_class_add_private (klass, sizeof (BurnerMetadataPrivate));

	object_class->finalize = burner_metadata_finalize;
	object_class->set_property = burner_metadata_set_property;
	object_class->get_property = burner_metadata_get_property;

	burner_metadata_signals[COMPLETED_SIGNAL] =
	    g_signal_new ("completed",
			  G_TYPE_FROM_CLASS (klass),
			  G_SIGNAL_RUN_LAST,
			  G_STRUCT_OFFSET (BurnerMetadataClass,
					   completed),
			  NULL, NULL,
...
BurnerMetadata *
burner_metadata_new (void)
{
	return BURNER_METADATA (g_object_new (BURNER_TYPE_METADATA, NULL));
}
