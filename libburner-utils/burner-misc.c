...
GQuark
burner_utils_error_quark (void)
{
	static GQuark quark = 0;

	if (!quark)
		quark = g_quark_from_static_string ("Burner_utils_error");
...
static const GOptionEntry options [] = {
	{ "burner-utils-debug", 'g', 0, G_OPTION_ARG_NONE, &use_debug,
	  N_("Display debug statements on stdout for Burner utilities library"),
	  NULL },
	{ NULL }
};
...
burner_utils_get_option_group (void)
{
	...
	group = g_option_group_new ("burner-utils",
				    N_("Burner utilities library"),
				    N_("Display options for Burner-utils library"),
				    NULL,
				    NULL);
			     ...
			     ...)
{
	...
	format_real = g_strdup_printf ("BurnerUtils: (at %s) %s\n",
				       location,
				       format);
...
static void
burner_utils_free (void)
{
	if (stringsH) {
		G_LOCK (stringsH);
		g_hash_table_foreach_remove (stringsH,
					     (GHRFunc) burner_utils_clear_strings_cb,
					     NULL);
...
burner_utils_get_uri_name (const gchar *uri)
{
	...
	utf8_name = burner_utils_validate_utf8 (name);
	if (utf8_name) {
		g_free (name);
		return utf8_name;
	}
...
GtkWidget *
burner_utils_properties_get_label (GtkWidget *properties)
{
	GList *children;
	GList *iter;
...
burner_utils_pack_properties (const gchar *title, ...)
{
	...
	result = burner_utils_pack_properties_list (title, list);
	g_slist_free (list);
				     ...
				     GtkMessageType type)
{
	...
	gtk_window_set_icon_name (GTK_WINDOW (message),
	                          parent? gtk_window_get_icon_name (GTK_WINDOW (parent)):"burner");
			      ...
			      GtkMessageType type)
{
	...
	message = burner_utils_create_message_dialog (parent,
						       primary_message,
						       secondary_message,
						       type);
