...
BurnerCLI cmd_line_options;

static gboolean
burner_cli_copy_project (const gchar *option_name,
                          const gchar *value,
                          gpointer data,
                          GError **error);
static gboolean
burner_cli_image_project (const gchar *option_name,
                           const gchar *value,
                           gpointer data,
                           GError **error);
...
//	  N_("PLAYLIST") },
	...
	{ "device", 0, G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, burner_cli_burning_device,
	  N_("Set the drive to be used for burning"),
	  N_("DEVICE PATH") },

	{ "image-file", 0, G_OPTION_FLAG_NO_ARG, G_OPTION_ARG_CALLBACK, burner_cli_fake_device,
...
//	  NULL },
	...
	{ "copy", 'c', G_OPTION_FLAG_OPTIONAL_ARG|G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, burner_cli_copy_project,
	  N_("Copy a disc"),
	  N_("PATH TO DEVICE") },
...
//	  NULL },
	...
	{ "image", 'i', G_OPTION_FLAG_OPTIONAL_ARG|G_OPTION_FLAG_FILENAME, G_OPTION_ARG_CALLBACK, burner_cli_image_project,
	 N_("URI of an image file to burn (autodetected)"),
          N_("PATH TO IMAGE") },

    	{ "empty", 'e', 0, G_OPTION_ARG_NONE, &cmd_line_options.empty_project,
         N_("Force Burner to display the project selection page"),
          NULL },
                         ...
                         GError **error)
{
	...
	/* Wait for the libburner-media to be ready */
	monitor = burner_medium_monitor_get_default ();
	while (burner_medium_monitor_is_probing (monitor))
		sleep (1);

	list = burner_medium_monitor_get_drives (monitor, BURNER_DRIVE_TYPE_FILE);
	if (!list)
		return FALSE;
                            ...
                            GError **error)
{
	...
	burner = burner_medium_monitor_get_drive (monitor, value);
	g_object_unref (monitor);

	if (burner) {
		if (!burner_drive_can_write (burner)) {
			gchar *string;

			/* Translators: %s is the path of drive */
			string = g_strdup_printf (_("\"%s\" cannot write."), value);
			burner_utils_message_dialog (NULL,
...
burner_cli_apply_options (BurnerApp *app)
{
	...
	if (cmd_line_options.parent_window)
		burner_app_set_parent (app, cmd_line_options.parent_window);

    	if (cmd_line_options.empty_project) {
	    	burner_app_create_mainwin (app);
		manager = burner_app_get_project_manager (app);
		burner_project_manager_empty (BURNER_PROJECT_MANAGER (manager));
		burner_app_run_mainwin (app);
		return;
	}
	...
	if (nb > 1) {
		...
		res = burner_app_open_project (app,
		                                cmd_line_options.burner,
		...
		burner_app_burn_uri (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.disc_blank) {
		burner_app_blank (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.disc_check) {
		burner_app_check (app, cmd_line_options.burner, cmd_line_options.burn_immediately != 0);
		return;
	}
	else if (cmd_line_options.audio_project) {
		burner_app_stream (app, cmd_line_options.burner, cmd_line_options.files, FALSE, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
	else if (cmd_line_options.data_project) {
		burner_app_data (app, cmd_line_options.burner, cmd_line_options.files, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
	else if (cmd_line_options.video_project) {
		burner_app_stream (app, cmd_line_options.burner, cmd_line_options.files, TRUE, cmd_line_options.burn_immediately != 0);
		if (cmd_line_options.burn_immediately)
			return;
	}
		...
		if (g_strv_length (cmd_line_options.files) == 1) {
			...
			result = burner_app_open_uri_drive_detection (app,
			                                               cmd_line_options.burner,
			...
			result = burner_app_open_uri (app, cmd_line_options.files [0], FALSE);
			if (!result)
				burner_app_data (app, cmd_line_options.burner, cmd_line_options.files, cmd_line_options.burn_immediately != 0);
		}
		else
