...
typedef struct _BurnerDataDiscPrivate BurnerDataDiscPrivate;
struct _BurnerDataDiscPrivate
{
	GtkWidget *tree;
	GtkWidget *filter;
	BurnerTrackDataCfg *project;
...
#define BURNER_DATA_DISC_PRIVATE(o)  (G_TYPE_INSTANCE_GET_PRIVATE ((o), BURNER_TYPE_DATA_DISC, BurnerDataDiscPrivate))


static void
burner_data_disc_new_folder_clicked_cb (GtkAction *action,
					 BurnerDataDisc *disc);
static void
burner_data_disc_open_activated_cb (GtkAction *action,
				     BurnerDataDisc *disc);
static void
burner_data_disc_rename_activated_cb (GtkAction *action,
				       BurnerDataDisc *disc);
static void
burner_data_disc_delete_activated_cb (GtkAction *action,
				       BurnerDataDisc *disc);
static void
burner_data_disc_paste_activated_cb (GtkAction *action,
				      BurnerDataDisc *disc);

static GtkActionEntry entries [] = {
	{"ContextualMenu", NULL, N_("Menu")},
	{"OpenFile", GTK_STOCK_OPEN, NULL, NULL, N_("Open the selected files"),
	 G_CALLBACK (burner_data_disc_open_activated_cb)},
	{"RenameData", NULL, N_("R_ename…"), NULL, N_("Rename the selected file"),
	 G_CALLBACK (burner_data_disc_rename_activated_cb)},
	{"DeleteData", GTK_STOCK_REMOVE, NULL, NULL, N_("Remove the selected files from the project"),
	 G_CALLBACK (burner_data_disc_delete_activated_cb)},
	{"PasteData", NULL, N_("Paste files"), NULL, N_("Add the files stored in the clipboard"),
	 G_CALLBACK (burner_data_disc_paste_activated_cb)},
//	{"NewFolder", "folder-new", N_("New _Folder"), NULL, N_("Create a new empty folder"),
	{"NewFolder", "newfolder-bt", N_("New _Folder"), NULL, N_("Create a new empty folder"),
	 G_CALLBACK (burner_data_disc_new_folder_clicked_cb)},
};
...
static GtkTargetEntry ntables_cd [] = {
	{BURNER_DND_TARGET_DATA_TRACK_REFERENCE_LIST, GTK_TARGET_SAME_WIDGET, TREE_MODEL_ROW},
...
static void
burner_data_disc_iface_disc_init (BurnerDiscIface *iface);

G_DEFINE_TYPE_WITH_CODE (BurnerDataDisc,
			 burner_data_disc,
			 GTK_TYPE_BOX,
			 G_IMPLEMENT_INTERFACE (BURNER_TYPE_DISC,
					        burner_data_disc_iface_disc_init));

#define BURNER_DATA_DISC_MEDIUM		"burner-data-disc-medium"
#define BURNER_DATA_DISC_MERGE_ID		"burner-data-disc-merge-id"
#define BURNER_MEDIUM_GET_UDI(medium)		(burner_drive_get_device (burner_medium_get_drive (medium)))

BurnerMedium *
burner_data_disc_get_loaded_medium (BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	priv = BURNER_DATA_DISC_PRIVATE (self);
	return burner_track_data_cfg_get_current_medium (priv->project);
}
...
static void
burner_data_disc_import_failure_dialog (BurnerDataDisc *disc,
					 GError *error)
{
...
static gboolean
burner_data_disc_import_session (BurnerDataDisc *disc,
				  BurnerMedium *medium,
				  gboolean import)
{
	BurnerDataDiscPrivate *priv;

	priv = BURNER_DATA_DISC_PRIVATE (disc);

	if (import) {
		GError *error = NULL;

		if (!burner_track_data_cfg_load_medium (priv->project, medium, &error)) {
			burner_data_disc_import_failure_dialog (disc, error);
			return FALSE;
		}
	...
	burner_track_data_cfg_unload_current_medium (priv->project);
	return FALSE;
}

static void
burner_data_disc_import_session_cb (GtkToggleAction *action,
				     BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	BurnerMedium *medium;
	...
	medium = g_object_get_data (G_OBJECT (action), BURNER_DATA_DISC_MEDIUM);
	if (!medium)
		return;

	burner_notify_message_remove (priv->message, BURNER_NOTIFY_CONTEXT_MULTISESSION);
	res = burner_data_disc_import_session (self,
						medium,
						gtk_toggle_action_get_active (action));

	/* make sure the button reflects the current state */
	if (gtk_toggle_action_get_active (action) != res) {
		g_signal_handlers_block_by_func (action, burner_data_disc_import_session_cb, self);
		gtk_toggle_action_set_active (GTK_TOGGLE_ACTION (action), res);
		g_signal_handlers_unblock_by_func (action, burner_data_disc_import_session_cb, self);
	}
}

static GtkTreePath *
burner_data_disc_get_parent (BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	gtk_tree_model_get (GTK_TREE_MODEL (priv->project), &iter,
			    BURNER_DATA_TREE_MODEL_IS_LOADING, &is_loading,
			    -1);
	...
	gtk_tree_model_get (GTK_TREE_MODEL (priv->project), &iter,
			    BURNER_DATA_TREE_MODEL_IS_FILE, &is_file,
			    -1);
					 ...
					 BurnerDataDisc *disc)
{
	...
	parent = burner_data_disc_get_parent (disc);
	treepath = burner_track_data_cfg_add_empty_directory (BURNER_TRACK_DATA_CFG (priv->project), NULL, parent);
...
struct _BurnerClipData {
	BurnerDataDisc *disc;
	GtkTreeRowReference *reference;
};
typedef struct _BurnerClipData BurnerClipData;

static void
burner_data_disc_clipboard_text_cb (GtkClipboard *clipboard,
				     const char *text,
				     BurnerClipData *data)
{
	BurnerDataDiscPrivate *priv;
	...
	priv = BURNER_DATA_DISC_PRIVATE (data->disc);
	...
	while (*item) {
		if (**item != '\0') {
			...
			burner_track_data_cfg_add (BURNER_TRACK_DATA_CFG (priv->project),
						    uri,
						    parent);
...
static void
burner_data_disc_clipboard_targets_cb (GtkClipboard *clipboard,
					GdkAtom *atoms,
					gint n_atoms,
					BurnerClipData *data)
{
	if (burner_clipboard_selection_may_have_uri (atoms, n_atoms)) {
		gtk_clipboard_request_text (clipboard,
					    ...
					    burner_data_disc_clipboard_text_cb,
					    data);
		return;
	}
				      ...
				      BurnerDataDisc *disc)
{
	...
	gtk_clipboard_request_targets (clipboard,
				       (GtkClipboardTargetsReceivedFunc) burner_data_disc_clipboard_targets_cb,
				       data);
}
...
static void
burner_data_disc_name_editing_started_cb (GtkCellRenderer *renderer,
...
static void
burner_data_disc_name_editing_canceled_cb (GtkCellRenderer *renderer,
					    BurnerDataDisc *disc)
{
	BurnerDataDiscPrivate *priv;
...
static void
burner_data_disc_name_edited_cb (GtkCellRendererText *cellrenderertext,
				  gchar *path_string,
				  gchar *text,
				  BurnerDataDisc *self)
{
	...
	gtk_tree_model_get (GTK_TREE_MODEL (priv->project), &row,
			    BURNER_DATA_TREE_MODEL_NAME, &name,
			    -1);
	...
	/* NOTE: BurnerDataProject is where we handle name collisions,
	 * UTF-8 validity, ...
	 * Here if there is a name collision then rename gets aborted. */
	burner_track_data_cfg_rename (BURNER_TRACK_DATA_CFG (priv->project), text, path);
	gtk_tree_path_free (path);
}
...
static void
burner_data_disc_project_loading_cb (BurnerTrackDataCfg *project,
				      gdouble progress,
				      BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	message = burner_notify_get_message_by_context_id (priv->message, BURNER_NOTIFY_CONTEXT_LOADING);
	if (!message)
		return;

	/* we're not done yet update progress. */
	burner_disc_message_set_progress (BURNER_DISC_MESSAGE (message), progress);
}

static void
burner_data_disc_project_loaded_cb (BurnerTrackDataCfg *project,
				     GSList *errors,
				     BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	if (errors) {
		burner_disc_message_remove_buttons (BURNER_DISC_MESSAGE (message));

		burner_disc_message_set_primary (BURNER_DISC_MESSAGE (message),
						  _("The contents of the project changed since it was saved."));
		burner_disc_message_set_secondary (BURNER_DISC_MESSAGE (message),
						    _("Discard the current modified project ?"));

		gtk_info_bar_set_message_type (GTK_INFO_BAR (message), GTK_MESSAGE_WARNING);

		burner_disc_message_set_progress_active (BURNER_DISC_MESSAGE (message), FALSE);
		...
		burner_disc_message_add_errors (BURNER_DISC_MESSAGE (message),
						 priv->load_errors);
...
static gboolean
burner_data_disc_launch_image (gpointer data)
{
	gchar *uri = data;

	burner_app_image (burner_app_get_default (), NULL, uri, FALSE);
	g_free (uri);
...
burner_data_disc_image_uri_cb (BurnerTrackDataCfg *vfs,
				const gchar *uri,
				BurnerDataDisc *self)
{
	gint answer;
	...
	button = burner_utils_make_button (_("Burn as _Image"),
	                                    NULL,
					    "media-optical-burn",
	...
	g_idle_add (burner_data_disc_launch_image, g_strdup (uri));

	return BURNER_BURN_CANCEL;
}

static void
burner_data_disc_filter_expanded_cb (GtkExpander *expander,
				      BurnerDataDisc *self)
{
	GtkWidget *parent;
...
static void
burner_data_disc_unreadable_uri_cb (BurnerTrackDataCfg *vfs,
				     const GError *error,
				     const gchar *uri,
				     BurnerDataDisc *self)
{
...
static void
burner_data_disc_recursive_uri_cb (BurnerTrackDataCfg *vfs,
				    const gchar *uri,
				    BurnerDataDisc *self)
{
	gchar *name;
...
static void
burner_data_disc_unknown_uri_cb (BurnerTrackDataCfg *vfs,
				  const gchar *uri,
				  BurnerDataDisc *self)
{
	gchar *name;
...
static void
burner_data_disc_joliet_rename_cb (BurnerTrackDataCfg *project,
				    BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	GtkWidget *dialog;
	...
	if (answer == GTK_RESPONSE_YES)
		burner_track_data_add_fs (BURNER_TRACK_DATA (priv->project),
					   BURNER_IMAGE_FS_JOLIET);
	else
		burner_track_data_rm_fs (BURNER_TRACK_DATA (priv->project),
					  BURNER_IMAGE_FS_JOLIET);
}

static gboolean
burner_data_disc_name_collision_cb (BurnerTrackDataCfg *project,
				     const gchar *name,
				     BurnerDataDisc *self)
{
	gint answer;
...
static gboolean
burner_data_disc_2G_file_cb (BurnerTrackDataCfg *project,
			      const gchar *name,
			      BurnerDataDisc *self)
{
	gint answer;
...
static gboolean
burner_data_disc_deep_directory_cb (BurnerTrackDataCfg *project,
				     const gchar *name,
				     BurnerDataDisc *self)
{
	gint answer;
...
static gboolean
burner_data_disc_size_changed (gpointer user_data)
{
	goffset sectors;
	BurnerDataDisc *self;
	BurnerDataDiscPrivate *priv;

	self = BURNER_DATA_DISC (user_data);
	priv = BURNER_DATA_DISC_PRIVATE (self);

	burner_track_get_size (BURNER_TRACK (priv->project),
				&sectors,
				NULL);
...
static void
burner_data_disc_size_changed_cb (BurnerTrackDataCfg *project,
				   BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	if (!priv->size_changed_id)
		priv->size_changed_id = g_timeout_add (500,
						       burner_data_disc_size_changed,
						       self);
}

static void
burner_disc_disc_session_import_response_cb (GtkButton *button,
					      ...
					      BurnerDataDisc *self)
{
	...
	medium = g_object_get_data (G_OBJECT (button), BURNER_DATA_DISC_MEDIUM);
	res = burner_data_disc_import_session (self, medium, TRUE);

	action_name = g_strdup_printf ("Import_%s", BURNER_MEDIUM_GET_UDI (medium));
...
static GtkAction *
burner_data_disc_import_button_new (BurnerDataDisc *self,
				     BurnerMedium *medium)
{
	GtkWidget *import_bt;
	...
	volume_name = burner_volume_get_name (BURNER_VOLUME (medium));
...
*/
	...
	toggle_entry.callback = G_CALLBACK (burner_data_disc_import_session_cb);
...
static void
burner_data_disc_remove_available_medium (BurnerDataDisc *self,
                                           BurnerMedium *medium)
{
	int merge_id;
	GtkAction *action;
	...
	merge_id = GPOINTER_TO_INT (g_object_get_data (G_OBJECT (action), BURNER_DATA_DISC_MERGE_ID));
...
static void
burner_data_disc_session_available_cb (BurnerTrackDataCfg *session,
					BurnerMedium *medium,
					gboolean available,
					BurnerDataDisc *self)
{
	...
	if (available) {
		...
		g_signal_connect (BURNER_DISC_MESSAGE (message),
				  "response",
				  G_CALLBACK (burner_disc_disc_session_import_response_cb),
				  self);
	}
	else
		burner_data_disc_remove_available_medium (self, medium);
}

static void
burner_data_disc_session_loaded_cb (BurnerTrackDataCfg *session,
				     BurnerMedium *medium,
				     gboolean loaded,
				     BurnerDataDisc *self)
{
...
/**
 * BurnerDisc interface implementation
 */

static void
burner_data_disc_clear (BurnerDisc *disc)
{
	BurnerDataDiscPrivate *priv;
	...
	if (burner_track_data_cfg_get_current_medium (BURNER_TRACK_DATA_CFG (priv->project)))
		burner_track_data_cfg_unload_current_medium (BURNER_TRACK_DATA_CFG (priv->project));
	...
	priv->overburning = FALSE;
 	...
 	burner_notify_message_remove (priv->message, BURNER_NOTIFY_CONTEXT_SIZE);
	burner_notify_message_remove (priv->message, BURNER_NOTIFY_CONTEXT_LOADING);
	burner_notify_message_remove (priv->message, BURNER_NOTIFY_CONTEXT_MULTISESSION);

	burner_track_data_cfg_reset (priv->project);
}

static GSList *
burner_data_disc_convert_tree_paths_to_references (GtkTreeModel *model,
                                            	    GList *treepaths)
{
	GList *iter;
	GSList *retval = NULL;
...
static void
burner_data_disc_delete_selected (BurnerDisc *disc)
{
	BurnerDataDiscPrivate *priv;
	GtkTreeSelection *selection;
	...
	references = burner_data_disc_convert_tree_paths_to_references (GTK_TREE_MODEL (priv->project), list);
	g_list_foreach (list, (GFunc) gtk_tree_path_free, NULL);
	g_list_free (list);
	...
	for (iter = references; iter; iter = iter->next) {
		...
		burner_track_data_cfg_remove (BURNER_TRACK_DATA_CFG (priv->project), treepath);
...
burner_data_disc_add_uri (BurnerDisc *disc, const gchar *uri)
{
	BurnerDataDiscPrivate *priv;
	GtkTreePath *parent = NULL;
	...
	if (priv->reject_files)
		return BURNER_DISC_LOADING;

	parent = burner_data_disc_get_parent (BURNER_DATA_DISC (disc));
	if (burner_track_data_cfg_add (BURNER_TRACK_DATA_CFG (priv->project), uri, parent)) {
		gtk_tree_path_free (parent);
...
static void
burner_data_disc_message_response_cb (BurnerDiscMessage *message,
				       GtkResponseType response,
				       BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	burner_data_disc_clear (BURNER_DISC (self));
}

static void
burner_data_disc_sort_column_changed (GtkTreeSortable *sortable,
                                       BurnerDataDisc *disc)
{
...
static BurnerDiscResult
burner_data_disc_set_track (BurnerDataDisc *disc,
			     BurnerTrackDataCfg *track)
{
	BurnerMedium *loaded_medium;
	BurnerDataDiscPrivate *priv;
	...
	if ((sort_column == BURNER_DATA_TREE_MODEL_SIZE
	||   sort_column == BURNER_DATA_TREE_MODEL_MIME_DESC
	||   sort_column == BURNER_DATA_TREE_MODEL_NAME)
	...
	/* filtered files */
	priv->filter = burner_file_filtered_new (priv->project);
	burner_file_filtered_set_right_button_group (BURNER_FILE_FILTERED (priv->filter), priv->button_size);
	g_signal_connect (priv->filter,
			  "activate",
			  G_CALLBACK (burner_data_disc_filter_expanded_cb),
			  disc);
	...
	loaded_medium = burner_track_data_cfg_get_current_medium (track);
	if (!priv->import_group) {
		...
		list = burner_track_data_cfg_get_available_media (priv->project);
		for (iter = list; iter; iter = iter->next) {
			BurnerMedium *medium;
			GtkAction *action;

			medium = iter->data;
			action = burner_data_disc_import_button_new (BURNER_DATA_DISC (disc), medium);
			if (!action)
				continue;
	...
	g_signal_connect (track,
	                  "sort-column-changed",
	                  G_CALLBACK (burner_data_disc_sort_column_changed),
	                  disc);

	g_signal_connect (track,
			  "2G-file",
			  G_CALLBACK (burner_data_disc_2G_file_cb),
			  disc);
	g_signal_connect (track,
			  "deep-directory",
			  G_CALLBACK (burner_data_disc_deep_directory_cb),
			  disc);
	g_signal_connect (track,
			  "name-collision",
			  G_CALLBACK (burner_data_disc_name_collision_cb),
			  disc);
	g_signal_connect (track,
			  "joliet-rename",
			  G_CALLBACK (burner_data_disc_joliet_rename_cb),
			  disc);

	g_signal_connect (track,
			  "source-loading",
			  G_CALLBACK (burner_data_disc_project_loading_cb),
			  disc);
	g_signal_connect (track,
			  "source-loaded",
			  G_CALLBACK (burner_data_disc_project_loaded_cb),
			  disc);

	/* Use the BurnerTrack "changed" signal for size changes */
	g_signal_connect (track,
			  "changed",
			  G_CALLBACK (burner_data_disc_size_changed_cb),
			  disc);

	g_signal_connect (track,
			  "image-uri",
			  G_CALLBACK (burner_data_disc_image_uri_cb),
			  disc);
	g_signal_connect (track,
			  "unreadable-uri",
			  G_CALLBACK (burner_data_disc_unreadable_uri_cb),
			  disc);
	g_signal_connect (track,
			  "recursive-sym",
			  G_CALLBACK (burner_data_disc_recursive_uri_cb),
			  disc);
	g_signal_connect (track,
			  "unknown-uri",
			  G_CALLBACK (burner_data_disc_unknown_uri_cb),
			  disc);

	g_signal_connect (track,
			  "session-available",
			  G_CALLBACK (burner_data_disc_session_available_cb),
			  disc);
	g_signal_connect (track,
			  "session-loaded",
			  G_CALLBACK (burner_data_disc_session_loaded_cb),
			  disc);

	gtk_tree_view_set_model (GTK_TREE_VIEW (priv->tree), GTK_TREE_MODEL (track));

	status = burner_status_new ();
	burner_track_get_status (BURNER_TRACK (track), status);
	result = burner_status_get_result (status);
	if (result == BURNER_BURN_OK || result == BURNER_BURN_RUNNING) {
	...
	if (result != BURNER_BURN_NOT_READY) {
		g_object_unref (status);
		return BURNER_DISC_ERROR_UNKNOWN;
	}
	...
	message = burner_notify_message_add (priv->message,
					      ...
					      NULL,
					      -1,
					      BURNER_NOTIFY_CONTEXT_LOADING);

	gtk_info_bar_set_message_type (GTK_INFO_BAR (message), GTK_MESSAGE_INFO);
	burner_disc_message_set_progress (BURNER_DISC_MESSAGE (message),
	...
	g_signal_connect (message,
			  "response",
			  G_CALLBACK (burner_data_disc_message_response_cb),
			  disc);
...
static void
burner_data_disc_unset_track (BurnerDataDisc *disc)
{
	BurnerDataDiscPrivate *priv;
	...
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_sort_column_changed,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_2G_file_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_deep_directory_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_name_collision_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_joliet_rename_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_project_loading_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_project_loaded_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_size_changed_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_image_uri_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_unreadable_uri_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_recursive_uri_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_unknown_uri_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_session_available_cb,
					      disc);
	g_signal_handlers_disconnect_by_func (priv->project,
					      burner_data_disc_session_loaded_cb,
					      disc);
...
static void
burner_data_disc_track_removed (BurnerBurnSession *session,
				 BurnerTrack *track,
				 guint former_position,
				 BurnerDataDisc *disc)
{
	g_signal_handlers_disconnect_by_func (session,
					      burner_data_disc_track_removed,
					      disc);

	burner_data_disc_unset_track (disc);
}

static BurnerDiscResult
burner_data_disc_set_session_contents (BurnerDisc *self,
					BurnerBurnSession *session)
{
	BurnerBurnResult result = BURNER_BURN_OK;
	GSList *tracks;

	burner_data_disc_unset_track (BURNER_DATA_DISC (self));
	...
	if (!tracks) {
		...
		/* If it's empty add one */
                ...
                burner_burn_session_add_track (session,
						BURNER_TRACK (data_track),
						NULL);
		burner_data_disc_set_track (BURNER_DATA_DISC (self),
					     BURNER_TRACK_DATA_CFG (data_track));

		/* NOTE: that track was reffed in burner_data_disc_set_track () */
		...
		if (BURNER_IS_TRACK_DATA_CFG (track))
			result = burner_data_disc_set_track (BURNER_DATA_DISC (self),
							      BURNER_TRACK_DATA_CFG (track));
	}

	g_signal_connect (session,
			  "track-removed",
			  G_CALLBACK (burner_data_disc_track_removed),
			  self);
...
static gboolean
burner_data_disc_get_selected_uri (BurnerDisc *disc,
				    gchar **uri)
{
	BurnerDataDiscPrivate *priv;
	GtkTreePath *path;
...
static guint
burner_data_disc_add_ui (BurnerDisc *disc,
			  GtkUIManager *manager,
			  GtkWidget *message)
{
	BurnerDataDiscPrivate *priv;
	...
	if (!priv->disc_group) {
		priv->disc_group = gtk_action_group_new (BURNER_DISC_ACTION "-data");
...
static void
burner_data_disc_open_file (BurnerDataDisc *disc, GList *list)
{
	GList *item;
	GSList *uris;
	BurnerDataDiscPrivate *priv;
	...
	for (item = list; item; item = item->next) {
		...
		gtk_tree_model_get (GTK_TREE_MODEL (priv->project), &iter,
				    BURNER_DATA_TREE_MODEL_IS_IMPORTED, &is_imported,
				    -1);
		if (is_imported)
			continue;
				     ...
				     BurnerDataDisc *disc)
{
	...
	selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (priv->tree));
	list = gtk_tree_selection_get_selected_rows (selection, NULL);
	burner_data_disc_open_file (disc, list);
...
static gboolean
burner_data_disc_mass_rename_cb (GtkTreeModel *model,
				  GtkTreeIter *iter,
				  GtkTreePath *treepath,
				  const gchar *old_name,
				  const gchar *new_name)
{
	return burner_track_data_cfg_rename (BURNER_TRACK_DATA_CFG (model),
					      new_name,
					      treepath);
}

static void
burner_data_disc_rename_activated (BurnerDataDisc *disc)
{
	BurnerDataDiscPrivate *priv;
	GtkTreeSelection *selection;
	GtkTreeViewColumn *column;
	...
	if (g_list_length (list) == 1) {
		...
		dialog = gtk_dialog_new_with_buttons (_("File Renaming"),
						      ...
						      NULL);
		rename = burner_rename_new ();
		burner_rename_set_show_keep_default (BURNER_RENAME (rename), FALSE);
		gtk_widget_show (rename);

		string = g_strdup_printf ("<b>%s</b>", _("Renaming mode"));
		frame = burner_utils_pack_properties (string, rename, NULL);
		g_free (string);
...
burner_data_disc_rename_activated_cb (GtkAction *action,
				       BurnerDataDisc *disc)
{
	burner_data_disc_rename_activated (disc);
}
...
burner_data_disc_delete_activated_cb (GtkAction *action,
				       BurnerDataDisc *disc)
{
	burner_data_disc_delete_selected (BURNER_DISC (disc));
}
...
static void
burner_data_disc_selection_changed_cb (GtkTreeSelection *selection,
					BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	GtkTreeModel *model;
...
static gboolean
burner_data_disc_tree_select_function (GtkTreeSelection *selection,
					GtkTreeModel *model,
					GtkTreePath *treepath,
...
static void
burner_data_disc_show_menu (int nb_selected,
			     GtkUIManager *manager,
...
static gboolean
burner_data_disc_button_pressed_cb (GtkTreeView *tree,
				     GdkEventButton *event,
				     BurnerDataDisc *self)
{
	GtkTreeIter iter;
	...
	if (event->button == 1) {
		...
		if (event->type == GDK_2BUTTON_PRESS) {
			if (treepath) {
				...
				burner_data_disc_open_file (self, list);
				g_list_free (list);
			}
		}
		else if (!result) {
		...
		selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (priv->tree));
		burner_data_disc_show_menu (gtk_tree_selection_count_selected_rows (selection),
					     priv->manager,
					     event);
	}
...
static gboolean
burner_data_disc_key_released_cb (GtkTreeView *tree,
				   GdkEventKey *event,
				   BurnerDataDisc *self)
{
	BurnerDataDiscPrivate *priv;
	...
	if (event->keyval == GDK_KEY_KP_Delete || event->keyval == GDK_KEY_Delete)
		burner_data_disc_delete_selected (BURNER_DISC (self));
	else if (event->keyval == GDK_KEY_F2)
		burner_data_disc_rename_activated (self);
...
void
burner_data_disc_set_right_button_group (BurnerDataDisc *self,
 ...
 */
static void
burner_data_disc_init (BurnerDataDisc *object)
{
	BurnerDataDiscPrivate *priv;
	GtkTreeSelection *selection;
	GtkTreeViewColumn *column;
	...
	priv = BURNER_DATA_DISC_PRIVATE (object);
	...
	g_signal_connect (priv->tree,
			  "button-press-event",
			  G_CALLBACK (burner_data_disc_button_pressed_cb),
			  object);
	
	g_signal_connect (priv->tree,
			  "key-release-event",
			  G_CALLBACK (burner_data_disc_key_released_cb),
			  object);

	selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (priv->tree));
	g_signal_connect (selection,
			  "changed",
			  G_CALLBACK (burner_data_disc_selection_changed_cb),
			  object);

	gtk_tree_selection_set_mode (selection, GTK_SELECTION_MULTIPLE);
	gtk_tree_selection_set_select_function (selection,
						burner_data_disc_tree_select_function,
						NULL,
						NULL);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "icon-name", BURNER_DATA_TREE_MODEL_MIME_ICON);

	renderer = gtk_cell_renderer_text_new ();
	g_signal_connect (G_OBJECT (renderer), "edited",
			  G_CALLBACK (burner_data_disc_name_edited_cb), object);
	g_signal_connect (G_OBJECT (renderer), "editing-started",
			  G_CALLBACK (burner_data_disc_name_editing_started_cb), object);
	g_signal_connect (G_OBJECT (renderer), "editing-canceled",
			  G_CALLBACK (burner_data_disc_name_editing_canceled_cb), object);

	gtk_tree_view_column_pack_end (column, renderer, TRUE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "text", BURNER_DATA_TREE_MODEL_NAME);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "style", BURNER_DATA_TREE_MODEL_STYLE);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "foreground", BURNER_DATA_TREE_MODEL_COLOR);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "editable", BURNER_DATA_TREE_MODEL_EDITABLE);
	...
	gtk_tree_view_column_set_sort_column_id (column, BURNER_DATA_TREE_MODEL_NAME);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "text", BURNER_DATA_TREE_MODEL_SIZE);
	gtk_tree_view_column_set_title (column, _("Size"));

	gtk_tree_view_append_column (GTK_TREE_VIEW (priv->tree), column);
	gtk_tree_view_column_set_resizable (column, TRUE);
	gtk_tree_view_column_set_expand (column, FALSE);
	gtk_tree_view_column_set_sort_column_id (column, BURNER_DATA_TREE_MODEL_SIZE);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "text", BURNER_DATA_TREE_MODEL_MIME_DESC);
	gtk_tree_view_column_set_title (column, _("Description"));

	gtk_tree_view_append_column (GTK_TREE_VIEW (priv->tree), column);
	gtk_tree_view_column_set_resizable (column, TRUE);
	gtk_tree_view_column_set_expand (column, FALSE);
	gtk_tree_view_column_set_sort_column_id (column, BURNER_DATA_TREE_MODEL_MIME_DESC);
	...
	gtk_tree_view_column_add_attribute (column, renderer,
					    "visible", BURNER_DATA_TREE_MODEL_SHOW_PERCENT);
	gtk_tree_view_column_add_attribute (column, renderer,
					    "perc", BURNER_DATA_TREE_MODEL_PERCENT);
...
static void
burner_data_disc_finalize (GObject *object)
{
	BurnerDataDiscPrivate *priv;
	...
	G_OBJECT_CLASS (burner_data_disc_parent_class)->finalize (object);
}

static gboolean
burner_data_disc_is_empty (BurnerDisc *disc)
{
	BurnerDataDiscPrivate *priv;
	GtkTreeModel *model;
...
burner_data_disc_iface_disc_init (BurnerDiscIface *iface)
{
	iface->add_uri = burner_data_disc_add_uri;
	iface->delete_selected = burner_data_disc_delete_selected;
	iface->is_empty = burner_data_disc_is_empty;
	iface->clear = burner_data_disc_clear;
	iface->set_session_contents = burner_data_disc_set_session_contents;
	iface->get_selected_uri = burner_data_disc_get_selected_uri;
	iface->add_ui = burner_data_disc_add_ui;
}

static void
burner_data_disc_get_property (GObject * object,
				guint prop_id,
				GValue * value,
				GParamSpec * pspec)
{
...
static void
burner_data_disc_set_property (GObject * object,
				guint prop_id,
				const GValue * value,
				GParamSpec * pspec)
{
...
static void
burner_data_disc_class_init (BurnerDataDiscClass *klass)
{
	GObjectClass* object_class = G_OBJECT_CLASS (klass);
	g_type_class_add_private (klass, sizeof (BurnerDataDiscPrivate));

	object_class->finalize = burner_data_disc_finalize;
	object_class->set_property = burner_data_disc_set_property;
	object_class->get_property = burner_data_disc_get_property;
...
{
	return GTK_WIDGET (g_object_new (BURNER_TYPE_DATA_DISC, NULL));
}
