...
typedef struct _BurnerGioOperation BurnerGioOperation;
struct _BurnerGioOperation
{
	GMainLoop *loop;
	GCancellable *cancel;
	guint timeout_id;
...
static void
burner_gio_operation_destroy (BurnerGioOperation *operation)
{
	if (operation->cancel) {
...
static void
burner_gio_operation_end (gpointer callback_data)
{
	BurnerGioOperation *operation = callback_data;
...
static gboolean
burner_gio_operation_timeout (gpointer callback_data)
{
	BurnerGioOperation *operation = callback_data;

	burner_gio_operation_end (operation);

	BURNER_MEDIA_LOG ("Volume/Disc operation timed out");
...
static void
burner_gio_operation_cancelled (GCancellable *cancel,
				 BurnerGioOperation *operation)
{
	operation->result = FALSE;
	g_cancellable_cancel (operation->cancel);
	if (operation->loop && g_main_loop_is_running (operation->loop))
...
static gboolean
burner_gio_operation_wait_for_operation_end (BurnerGioOperation *operation,
					      GError **error)
{
	BURNER_MEDIA_LOG ("Waiting for end of async operation");

	g_object_ref (operation->cancel);
	g_cancellable_reset (operation->cancel);
	g_signal_connect (operation->cancel,
			  "cancelled",
			  G_CALLBACK (burner_gio_operation_cancelled),
			  operation);
	...
	if (operation->error) {
		BURNER_MEDIA_LOG ("Medium operation finished with an error %s",
				   operation->error->message);

		if (operation->error->code == G_IO_ERROR_FAILED_HANDLED) {
			BURNER_MEDIA_LOG ("Error already handled and displayed by GIO");
...
static void
burner_gio_operation_umounted_cb (GMount *mount,
				   BurnerGioOperation *operation)
{
	burner_gio_operation_end (operation);
}
...
static void
burner_gio_operation_umount_finish (GObject *source,
				     GAsyncResult *result,
				     gpointer user_data)
{
	BurnerGioOperation *op = user_data;
	...
	BURNER_MEDIA_LOG ("Umount operation completed (result = %d)", op->result);
			      ...
			      GError **error)
{
	...
	BURNER_MEDIA_LOG ("Unmounting volume");

	if (!gvolume) {
		BURNER_MEDIA_LOG ("No volume");
		return TRUE;
	}

	mount = g_volume_get_mount (gvolume);
	if (!mount) {
		BURNER_MEDIA_LOG ("No mount");
		return TRUE;
	}
	...
	if (!g_mount_can_unmount (mount)) {
		 ...
		 * mounted on burn:// */
		BURNER_MEDIA_LOG ("Mount can't unmount");
		return FALSE;
	}

	if (wait) {
		gulong umount_sig;
		BurnerGioOperation *op;

		op = g_new0 (BurnerGioOperation, 1);
		op->cancel = cancel;

		umount_sig = g_signal_connect_after (mount,
						     "unmounted",
						     G_CALLBACK (burner_gio_operation_umounted_cb),
						     op);
		...
		g_mount_unmount_with_operation (mount,
						...
						NULL,
				 		cancel,
				 		burner_gio_operation_umount_finish,
				 		op);
		result = burner_gio_operation_wait_for_operation_end (op, error);
		burner_gio_operation_destroy (op);
	...
	BURNER_MEDIA_LOG ("Unmount result = %d", result);
	return result;
}

static void
burner_gio_operation_mount_finish (GObject *source,
				    GAsyncResult *result,
				    gpointer user_data)
{
	BurnerGioOperation *op = user_data;
			     ...
			     GError **error)
{
	...
	if (!g_volume_can_mount (gvolume)) {
		BURNER_MEDIA_LOG ("Volume can't be mounted");
		return FALSE;
	}

	mount = g_volume_get_mount (gvolume);
	if (mount) {
		BURNER_MEDIA_LOG ("Existing mount");
		g_object_unref (mount);
		return TRUE;
	}
	...
	if (wait) {
		...
		g_volume_mount (gvolume,
				...
				cancel,
				burner_gio_operation_mount_finish,
				op);
		result = burner_gio_operation_wait_for_operation_end (op, error);
	...
	BURNER_MEDIA_LOG ("Mount result = %d", result);

	return result;
}

static void
burner_gio_operation_eject_finish (GObject *source,
				    GAsyncResult *result,
				    gpointer user_data)
{
	BurnerGioOperation *operation = user_data;
...
static void
burner_gio_operation_removed_cb (GVolume *volume,
				    ...
				    GError **error)
{
	...
	if (!g_volume_can_eject (gvolume)) {
		BURNER_MEDIA_LOG ("GVolume cannot be ejected");
		return FALSE;
	}
	...
	if (wait) {
		...
		eject_sig = g_signal_connect (gvolume,
					      "removed",
					      G_CALLBACK (burner_gio_operation_removed_cb),
					      op);
		...
		g_volume_eject_with_operation (gvolume,
					       ...
					       NULL,
					       cancel,
				  	       burner_gio_operation_eject_finish,
					       op);
...
static void
burner_gio_operation_ejected_cb (GDrive *gdrive,
				  gpointer callback_data)
{
	BurnerGioOperation *operation = callback_data;
...
static void
burner_gio_operation_disconnected_cb (GDrive *gdrive,
				       gpointer callback_data)
{
	BurnerGioOperation *operation = callback_data;
	burner_gio_operation_end (operation);
				   ...
				   GError **error)
{
	...
	if (wait) {
		...
		eject_sig = g_signal_connect (gdrive,
					      "changed",
					      G_CALLBACK (burner_gio_operation_ejected_cb),
					      op);

		disconnect_sig = g_signal_connect (gdrive,
						   "disconnected",
						   G_CALLBACK (burner_gio_operation_disconnected_cb),
						   op);
